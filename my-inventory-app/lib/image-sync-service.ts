//lib/image-sync-service.ts
import { existsSync, mkdirSync, writeFileSync } from 'fs';
import { join } from 'path';
import { Octokit } from 'octokit';
import { decryptToken } from '@/lib/decryption/decrypt-token';

export class ImageSyncService {
  private octokit: Octokit | null = null;

  constructor() {
    // Инициализация отложенная - в getOctokit()
  }
  ...
  private async syncImageFromGitHub(imagePath: string): Promise<void> {
    ...
    try {
      const octokit = await this.getOctokit(); // ← ИСПОЛЬЗУЕМ ДЕШИФРОВАННЫЙ ТОКЕН
  ...
  private async findImageInDatabase(imagePath: string) {
    ...
    // TODO: Добавить Prisma запрос когда будет БД
    return null;
  }
}